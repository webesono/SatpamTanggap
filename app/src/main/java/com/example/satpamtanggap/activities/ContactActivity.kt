...
import com.example.satpamtanggap.databinding.ActivityContactBinding
            ...
            .addOnCompleteListener{
                ...
                if (it.isSuccessful && it.result != null){
                    ...
                    for (queryDocumentSnapshot: QueryDocumentSnapshot in it.result){
                        ...
                        val user = User()
                        user.nama = queryDocumentSnapshot.getString(Constants.KEY_NAME).toString()
                        user.alamat = queryDocumentSnapshot.getString(Constants.KEY_ADDRESS).toString()
                        ...
                        user.id = queryDocumentSnapshot.getString(Constants.KEY_USER_ID).toString()
                        users.add(user)
                    }
                    if (users.size > 0){
